...
public class WindowTable extends JPanel implements PropertyChangeListener{	
	...
	public WindowTable(BudgetModel model, BudgetController controller){
	    ...
	    sorter.addRowSorterListener(e -> tempTotals());
	...
	public void filterCombined() {
		 ...
		 if (filterLibelle != null) {
			 filtered_rows.add(RowFilter.regexFilter("^" + Pattern.quote(filterLibelle) + "$", 1));
		 }
	...
	private void tempTotals() {
		AccountTempSums summary = model.tempSum(sorter);
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		tempTotals();  //affiche les totaux temporaires selon les lignes affichées
		
		labelBalance.setText("CHF " + String.valueOf(model.getAccountSolde()));	//affiche le total général selon les objects account
		...
		// Vérifie si la date est déjà dans le combo
     	...
     	for (int i = 0; i < comboxDates.getItemCount(); i++) {
     	    if (comboxDates.getItemAt(i).equals(date)) {
     	    	date_exists = true;
     	        break;
     	    }
     	}  	
     	...
     	for (int i=0 ; i < comboxLibelles.getItemCount(); i++) {
     		if (comboxLibelles.getItemAt(i).equals(libel)) {
	     		libelle_exists = true;
	     		break;
     		}
     	}
