...
public class BudgetModel extends AbstractTableModel implements Serializable {
    ...
    //on communique avec la classe et les variables pour calculer les sommes temporaires des lignes affichées, mais pas le total
    public AccountTempSums tempSum(TableRowSorter<BudgetModel> sorter) {
        float sumCredit = 0f;
        float sumDebit  = 0f;
        ...
        for (int v = 0; v < sorter.getViewRowCount(); v++) {
            ...
            Float credit = (Float) getValueAt(m, 2);
            Float debit = (Float)getValueAt(m, 3);
        ...
        return new AccountTempSums(sumCredit, sumDebit);

}
    public void saveBudget(File file) {
    ...
    public static BudgetModel loadFromFile(File file) {
	    try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(file))) {
	        return (BudgetModel) in.readObject();
